    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[1]) {
            case "add": {
                ...
                clan.sendMessage(Message.CAPTAINS_COMMAND.ADDED.replaceAll("%targetName%", target.getName()).replaceAll("%playerName%", player.getName()));
                break;
            }
            case "remove": {
                if (args.length != 3) {
                ...
                ClanHandler.save(clan);
                clan.sendMessage(Message.CAPTAINS_COMMAND.REMOVED.replaceAll("%targetName%", target.getName()).replaceAll("%playerName%", player.getName()));
                break;
            }
            default:
                Message.send(player, Message.CAPTAINS_COMMAND.USAGE.replaceAll("%label%", label));
