            ...
            for (String message : Message.LIST_COMMAND.LISTED) {
                if (message.equals("%clans%")) {
                    for (int i = minClan; i < (Math.min(maxClan, sorted.size())); i++) {
                        ...
                        BaseComponent[] comp = MiniMessageParser.parseFormat(Message.LIST_COMMAND.CLAN_FORMAT
                                        .replaceAll("%prefix%", playerClan != null && playerClan == clan ?
                                                Message.LIST_COMMAND.PREFIX.OWN : clan.getOnlineMembersAsList(player).size() < 1 ? Message.LIST_COMMAND.PREFIX.ENEMY :
                                                Message.LIST_COMMAND.PREFIX.ENEMY_ONLINE)
        ...
        for (String message : Message.LIST_COMMAND.LISTED) {
            if (message.equals("%clans%")) {
                for (int i = minClan; i < (Math.min(maxClan, sorted.size())); i++) {
                    Clan clan = sorted.get(i);
                    BaseComponent[] comp = MiniMessageParser.parseFormat(Message.LIST_COMMAND.CLAN_FORMAT
                            .replaceAll("%prefix%", clan.getOnlineMembersAsList().size() < 1 ? Message.LIST_COMMAND.PREFIX.ENEMY :
                                    Message.LIST_COMMAND.PREFIX.ENEMY_ONLINE)
                            .replaceAll("%id%", clan.getId().toString())
                            .replaceAll("%name%", clan.getName())
                            .replaceAll("%points%", String.valueOf(clan.getPoints()))
...
//                    sender.sendMessage(Message.LIST_COMMAND.CLAN_FORMAT
//                            .replaceAll("%prefix%", clan.getOnlineMembersAsList().size() < 1 ? Message.LIST_COMMAND.PREFIX.ENEMY :
//                                    Message.LIST_COMMAND.PREFIX.ENEMY_ONLINE)
//                            .replaceAll("%id%", clan.getId().toString())
//                            .replaceAll("%name%", clan.getName())
